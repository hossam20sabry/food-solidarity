...
<div class="container my-4">
    ...
    <form action="{{ route('admin.FoodTypes.store')}}" method="post" id="create" class="d-none">
        @csrf
    ...
    <table class="table table-secondary">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$type->name}}</td>
                <td>@if($type->flag == 0) Dry @else Protein @endif</td>
                <td>
                    <a href="{{ route('admin.FoodTypes.edit', $type->id)}}">
                        <button class="btn btn-primary">Edit</button>
                    </a>
                </td>
                <td>
                    <form action="{{ route('admin.FoodTypes.destroy', $type->id)}}" method="post">
                        @csrf
                        @method('DELETE')
